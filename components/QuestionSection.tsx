    ...
    return (
        <div className="max-w-4xl w-full space-y-4">
            <div className="flex items-center justify-between border-b border-slate-800/50 pb-3">
                ...
                <button onClick={() => setSortBy(sortBy === 'newest' ? 'oldest' : 'newest')} className="flex items-center gap-1.5 px-2 py-1 rounded-md bg-slate-900/50 border border-slate-800 text-[10px] text-slate-500 hover:text-slate-300">
                    <ArrowUpDown size={10} />
            ...
            <div className="max-h-[400px] overflow-y-auto pr-2 space-y-3 scrollbar-thin scrollbar-thumb-slate-800">
                {loading ? (
                    ...
                    sortedQuestions.map((q) => (
                        <div key={q.id} className="bg-slate-900/20 border border-slate-800/40 p-3.5 rounded-xl">
                            <div className="flex flex-col gap-1.5 mb-3">
                                ...
                                <p className="text-[12px] font-medium text-slate-200">{q.question}</p>
                            </div>
                            {q.answer ? (
                                ...
                                <div className="flex items-center gap-2 text-[10px] text-amber-500/50 italic">
            ...
            <div className="relative mt-2">
                <input 
                    ...
                    className="w-full bg-slate-900/30 border border-slate-800 rounded-xl px-4 py-2.5 text-[11px] text-slate-300 focus:outline-none focus:border-blue-500/40 pr-10"
                />
                <button onClick={handleSendQuestion} disabled={!newQuestion || sending || !user} className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 text-blue-500">
