        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-10">
          <div className="space-y-3 bg-white/5 p-4 rounded-2xl">
            <label className="flex items-center justify-between cursor-pointer group">
              ...
              <input type="checkbox" checked={state.onlyInStock} onChange={() => setState.setOnlyInStock(!state.onlyInStock)} className="w-4 h-4 accent-blue-600 rounded" />
            </label>
            <label className="flex items-center justify-between cursor-pointer group">
              <span className="text-[10px] font-black text-slate-400 group-hover:text-white transition-colors flex items-center gap-2 uppercase tracking-tighter">
                <Percent size={14} className="text-blue-500" /> İndirimli Ürünler
              </span>
              <input type="checkbox" checked={state.onlyDiscounted} onChange={() => setState.setOnlyDiscounted(!state.onlyDiscounted)} className="w-4 h-4 accent-blue-600 rounded" />
            </label>
          </div>

          <FilterSection title="Fiyat Aralığı (₺)">
            <div className="flex items-center gap-2 mt-4">
              <input type="number" placeholder="Min" value={state.minPrice} onChange={(e) => setState.setMinPrice(e.target.value)} className="w-full bg-gray-900 border border-white/5 rounded-xl px-3 py-2.5 text-[11px] font-bold focus:border-blue-500 outline-none text-white" />
              <div className="w-4 h-[1px] bg-slate-800 shrink-0" />
              <input type="number" placeholder="Max" value={state.maxPrice} onChange={(e) => setState.setMaxPrice(e.target.value)} className="w-full bg-gray-900 border border-white/5 rounded-xl px-3 py-2.5 text-[11px] font-bold focus:border-blue-500 outline-none text-white" />
            </div>
          </FilterSection>
          ...
          <FilterSection title="Cinsiyet">
            <div className="flex flex-wrap gap-2 mt-4">
              {filters.genders.map((g: string) => (
                <FilterBadge key={g} label={g} active={state.selectedGenders.includes(g)} onClick={() => setState.setSelectedGenders((prev: string[]) => prev.includes(g) ? prev.filter(i => i !== g) : [...prev, g])} />
              ))}
            </div>
          </FilterSection>
          ...
          <FilterSection title="Markalar">
            <div className="space-y-3 mt-4">
              {filters.brands.map((brand: string) => (
                <CheckboxItem key={brand} label={brand} checked={state.selectedBrands.includes(brand)} onChange={() => setState.setSelectedBrands((prev: string[]) => prev.includes(brand) ? prev.filter(i => i !== brand) : [...prev, brand])} />
              ))}
            </div>
          </FilterSection>
          ...
          <FilterSection title="Minimum Puan">
            <div className="grid grid-cols-3 gap-2 mt-4">
              {[3, 4, 4.5].map(r => (
                <button key={r} onClick={() => setState.setMinRating(state.minRating === r ? 0 : r)} className={`py-2.5 rounded-xl text-[10px] font-black border transition-all flex items-center justify-center gap-1 ${state.minRating === r ? 'bg-blue-600 border-blue-600 text-white shadow-lg shadow-blue-600/20' : 'bg-gray-900 border-white/5 text-slate-500 hover:border-white/10'}`}>
...
const FilterBadge = ({ label, active, onClick }: any) => (
  <button onClick={onClick} className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase transition-all border ${active ? 'bg-blue-600 text-white border-blue-600 shadow-lg shadow-blue-600/30' : 'bg-gray-900 text-slate-400 border-white/5 hover:border-white/10 hover:text-slate-200'}`}>
    {label}
  </button>
);
